...
function getStatus() {
    ...
    // 1. Internet Status
    if (internetStatus === 'offline') globalStatus = 'error';
    else if (internetStatus === 'partial') globalStatus = 'alert';

    // 2. Service Statuses
    for (const key in serviceStatus) {
        const s = serviceStatus[key];
        if (s.status === 'offline' || s.status === 'partial') {
            // Find if there's an override in config.services
            const cfg = (config.services || []).find(sc => 
                sc.key === key || `ping:${sc.key}` === key || `system:${sc.key}` === key || `storage:${sc.key}` === key
            );
            
            // Default to 'error' for normal service cards unless overridden
            const level = cfg?.setPageTitle?.triggerGlobalStatus || 'error';
            if (level === 'error') globalStatus = 'error';
            else if ((level === 'warning' || level === 'alert') && globalStatus !== 'error') globalStatus = 'alert';
        }
    }
    ...
    const pageTitles = (config.headerData || [])
        .map(item => {
            ...
            // 3. Header Item Directives
            if (activePageTitles[item.name]) {
                const level = item.setPageTitle?.triggerGlobalStatus;
                if (level === 'error') globalStatus = 'error';
                else if ((level === 'warning' || level === 'alert') && globalStatus !== 'error') globalStatus = 'alert';
